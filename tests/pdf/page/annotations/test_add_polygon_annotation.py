...
logging.basicConfig(
    filename=Path(get_log_dir(), "test-add-polygon-annotation.log"), level=logging.DEBUG
)


class TestAddPolygonAnnotation(unittest.TestCase):
    def __init__(self, methodName="runTest"):
        super().__init__(methodName)
        self.input_file = Path("/home/joris/Code/pdf-corpus/0203.pdf")
        self.output_file = Path(
            get_output_dir(), "test-add-polygon-annotation/output.pdf"
        )
