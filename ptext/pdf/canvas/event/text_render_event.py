...
class ChunkOfTextRenderEvent(Event, ChunkOfText):
    ...
    def __init__(self, graphics_state: CanvasGraphicsState, raw_bytes: String):
        ...
        uses_descent = any(
            [x in self.text for x in ["y", "p", "q", "f", "g", "j"]]
        )  # TODO
        if uses_descent:
            self.set_bounding_box(self.baseline_bounding_box)  # TODO
        else:
    ...
    def split_on_glyphs(self) -> typing.List["ChunkOfTextRenderEvent"]:
        ...
        for g in self._glyph_line.glyphs:
            ...
            e.set_bounding_box(Rectangle(p0[0], p0[1], p1[0] - p0[0], p1[1] - p0[1]))

            # set baseline box
            e.baseline_bounding_box = e.bounding_box
