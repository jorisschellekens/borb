...
def main(root: str):
    ...
    conformance_check_file: typing.Optional[pathlib.Path] = next(
        iter([x for x in done if x.stem == "conformance_checks"])
    )
    ...
    for line in lines:
        ...
        if m != prev_m:
            print(
                f"Number of checks changed: [{n}/{m}] found AFTER [{prev_n}/{prev_m}]"
            )
            failed = True
        prev_n = n
        prev_m = m
